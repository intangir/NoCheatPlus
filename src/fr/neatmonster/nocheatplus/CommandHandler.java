...
import fr.neatmonster.nocheatplus.checks.ViolationHistory.Violation;
...
public class CommandHandler implements CommandExecutor {
    ...
    private void handleInfoCommand(final CommandSender sender, final String playerName) {
        ...
        if (player != null) {
            ...
            final TreeMap<Long, Violation> violations = ViolationHistory.getHistory(player).getViolations();
            if (violations.size() > 0) {
                sender.sendMessage(TAG + "Displaying " + playerName + "'s violations...");
                for (final long time : violations.descendingKeySet()) {
                    final Violation violation = violations.get(time);
                    final String[] paths = violation.check.split("\\.");
                    final String checkName = paths[paths.length - 1];
                    final String parentName = paths[paths.length - 2];
                    final double violationLevel = Math.round(violation.VL);
                    sender.sendMessage(TAG + "[" + dateFormat.format(new Date(time)) + "] (" + parentName + ".)"
                            + checkName + " VL " + violationLevel);
                }
            } else
